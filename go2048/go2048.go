...
// ----------初始化游戏界面---------------
func (t G2048) tipcolor(ox, oy int,wcolor,wbcolor termbox.Attribute)error{
	
    //var color termbox.Attribute = termbox.ColorRed   
	fg := wcolor   //提示字的颜色设置
	bg := wbcolor //界面背景的颜色设置
	termbox.Clear(fg, bg)//绘制除提示字背景以外的颜色
		str := "      SCORE: " + fmt.Sprint(Score)
	for n, c := range str {
		termbox.SetCell(ox+n, oy-1, c, fg, bg)
	}
	str = "Quit:Esc key" + "|||" + "Replay:Enter key"
	for n, c := range str {
...
func (t G2048) tablecolor(ox, oy int,tcolor,lcolor termbox.Attribute)error{
	
	fg1 := lcolor  //纵横细线的颜色设置
	bg1 := tcolor //格子的颜色设置
	for i := 0; i <= len(t); i++ {
		for x := 0; x < 5*len(t); x++ {
			termbox.SetCell(ox+x, oy+i*2, '-', fg1, bg1)
		}
		for x := 0; x <= 2*len(t); x++ {
			if x%2 == 0 {
				termbox.SetCell(ox+i*5, oy+x, '+', fg1, bg1)
			} else {
				termbox.SetCell(ox+i*5, oy+x, '|', fg1, bg1)
			}
		}
...
func (t G2048)numbercolor(ox, oy int,ncolor,nbcolor termbox.Attribute)error{
	
	fg2 := ncolor //数字颜色设置
	bg2 := nbcolor //数字背景颜色设置
	for i := range t {
		for j := range t[i] {
			if t[i][j] > 0 {
				str := fmt.Sprint(t[i][j])
				for n, char := range str {
					termbox.SetCell(ox+j*5+1+n, oy+i*2+1, char, fg2, bg2)
				}
			}
		}
	}
...
func (t *G2048) mrgeAndReturnKey(wcolor,tcolor,ncolor,wbcolor,lcolor,nbcolor termbox.Attribute) termbox.Key {
	var changed bool
...
Lable:
	...
	case termbox.EventResize:
		x, y := termbox.Size()
		t.tipcolor(x/2-10, y/2-4,wcolor,wbcolor)	
		//t.tipcolor(x/2-10, y/2-4,termbox.ColorRed)	
		t.tablecolor(x/2-10, y/2-4,tcolor,lcolor)
		t.numbercolor(x/2-10, y/2-4,ncolor,nbcolor)
...
// 开始游戏
func (b *G2048) Run(wcolor,tcolor,ncolor,wbcolor,lcolor,nbcolor termbox.Attribute) {
	err := termbox.Init()
	if err != nil {
		panic(err)
	}
...
Restart:
	...
	for { // 进入无限循环
		st := b.checkWinOrAdd()
		x, y := termbox.Size()
		b.tipcolor(x/2-10, y/2-4,wcolor,wbcolor)	
		//b.tipcolor(x/2-10, y/2-4,termbox.ColorRed)	
		b.tablecolor(x/2-10, y/2-4,tcolor,lcolor)
		b.numbercolor(x/2-10, y/2-4,ncolor,nbcolor)
		...
		// 检查用户按键
		key := b.mrgeAndReturnKey(wcolor,tcolor,ncolor,wbcolor,lcolor,nbcolor)
		// 如果按键是 Esc 则退出游戏
